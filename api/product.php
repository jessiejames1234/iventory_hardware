...
function getProducts() {
    ...
    $sql = "SELECT 
            ...
            LEFT JOIN inventory i ON i.product_id = p.product_id AND i.location_id = :store_id
            WHERE 1=1";
    ...
    if ($brand !== '')          $sql .= " AND p.brand_id = :brand";
    if ($unit !== '')           $sql .= " AND p.unit_id = :unit";
    if ($search !== '')         $sql .= " AND (p.product_name LIKE :search OR p.model LIKE :search)";
    ...
    if ($unit !== '')     $stmt->bindValue(':unit', $unit, PDO::PARAM_INT);
    if ($search !== '') {
        $like = "%$search%";
    ...
    echo json_encode($stmt->fetchAll(PDO::FETCH_ASSOC));
        ...
        if (!empty($errors)) { echo json_encode(["status"=>"error","message"=>implode(" ",$errors)]); return; }
        ...
        if ($createdBy === null) { echo json_encode(["status"=>"error","message"=>"User not identified."]); return; }

        // duplicate product_name check
        ...
        $dupStmt->execute();
        if ($dupStmt->fetchColumn() > 0) { echo json_encode(["status"=>"error","message"=>"Product name already exists."]); return; }

        try {
            $conn->beginTransaction();

            // 1) Insert without SKU first
            $sql = "INSERT INTO product 
                    (product_name, model, specs, warranty_period, selling_price, cost_price, reorder_level, 
                    category_id, brand_id, unit_id, sku, created_by)
                    VALUES 
                    (:name, :model, :specs, :warranty, :price, :cost_price, :reorder_level,
                    :category_id, :brand_id, :unit_id, '', :created_by)";
            $stmt = $conn->prepare($sql);
            ...
            // 2) Build SKU = NAME(3)-CATEGORY(3)-BRAND(3)-NNN
            // fetch category & brand names
            $namesStmt = $conn->prepare("
            SELECT 
                (SELECT name FROM category WHERE category_id = :cid) AS cat_name,
                (SELECT name FROM brand    WHERE brand_id    = :bid) AS brand_name
            ");
            $namesStmt->execute([":cid" => (int)$catId, ":bid" => (int)$brandId]);
            $nb = $namesStmt->fetch(PDO::FETCH_ASSOC) ?: ["cat_name"=>"","brand_name"=>""];

            $slug3 = function(string $s): string {
            // keep letters/numbers only, remove spaces, uppercase, first 3
            $s = preg_replace('/[^A-Za-z0-9]/', '', $s ?? '');
            return strtoupper(substr($s, 0, 3));
            };

            $partName = $slug3($name);
            $partCat  = $slug3($nb['cat_name'] ?? '');
            $partBr   = $slug3($nb['brand_name'] ?? '');
            $seq      = str_pad((string)$newId, 3, '0', STR_PAD_LEFT);

            $sku = "{$partName}-{$partCat}-{$partBr}-{$seq}";
            if ($sku === "---{$seq}") { // ultra-defensive
            $sku = "SKU-{$seq}";
            }

            // extra safety: ensure uniqueness (rare if product_id is unique, but just in case)
            $check = $conn->prepare("SELECT COUNT(*) FROM product WHERE sku = :sku");
            $trySku = $sku;
            $i = 1;
            while (true) {
            $check->execute([":sku" => $trySku]);
            if ((int)$check->fetchColumn() === 0) break;
            $trySku = $sku . "-" . str_pad((string)$i, 2, '0', STR_PAD_LEFT);
            $i++;
            }
            $sku = $trySku;

            // 3) Save SKU
            $up = $conn->prepare("UPDATE product SET sku = :sku WHERE product_id = :id");
            $up->execute([":sku" => $sku, ":id" => $newId]);

            $conn->commit();
            echo json_encode(["status"=>"success","product_id"=>$newId,"sku"=>$sku]);
...
function getFilters() {
    ...
    try {
        ...
        // Categories
        $catStmt = $conn->query("SELECT category_id, name FROM category ORDER BY name ASC");
        $result['categories'] = $catStmt->fetchAll(PDO::FETCH_ASSOC);

        // Brands
        $brandStmt = $conn->query("SELECT brand_id, name FROM brand ORDER BY name ASC");
        $result['brands'] = $brandStmt->fetchAll(PDO::FETCH_ASSOC);

        // Units
        $unitStmt = $conn->query("SELECT unit_id, name FROM unit_tbl ORDER BY name ASC");
        $result['units'] = $unitStmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        switch ($operation) {
            ...
            case "toggleProductStatus":
                $product->toggleProductStatus($json);
                break;
